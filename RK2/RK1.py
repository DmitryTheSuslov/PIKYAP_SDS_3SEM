...
class RK1:
    def __init__(self):
        self.one_to_many = [(b.title, b.price, s.name)
                       for b in books
                       for s in shops
                       if b.shop_id == s.id]

        # Соединение данных многие-ко-многим
        self.many_to_many_temp = [(s.name, sh.shop_id, sh.book_id)
                             for s in shops
                             for sh in books_shops
                             if s.id == sh.shop_id]

        self.many_to_many = [(b.title, b.price, shop_name)
    ...
    def first(self):
        ...
        res1 = {name: [(a[0], a[1]) for a in self.one_to_many if a[2] == name] for name in correct_shop_names}
        return res1

    def second(self):
        shop_names = [sh.name for sh in shops]
        res2 = sorted([(name, max([a[1] for a in self.one_to_many if a[2] == name])) for name in shop_names],
                      key=lambda x: -x[1])
        return res2
    ...
    def third(self):
        ...
        for name in sorted(shop_names):
            s += '\n' + name + '\n'
            s += '\n'.join([(f'\t{a[0]} {a[1]}') for a in self.many_to_many if a[2] == name])
        return s
